...
var senators = data.results[0].members;
/*Declaramos un objeto con las estadisticas a usar en las tablas informativas*/
var statistics = {
  partyName1 : "Republicans",
  partyName2 : "Democrats",
  partyName3 : "Independents",
  quantityRepublicans : 0,
  quantityDemocrats: 0,
  quantityIndependents: 0,
  percentageOfVotesPerRepublicans : 0,
  percentageOfVotesPerDemocrats : 0,
  percentageOfVotesPerIndependents : 0,
  tenWithMostAttendance : 0,
  tenWithLeastAttendance : 0,
  tenWithMostLoyalty : 0,
  tenWithLeastLoyalty : 0,
}
/*Pasamos a formato JSON el objeto con las estadisticas*/
var stringStatistics = JSON.stringify(statistics);
/*Declaramos variables que van a almacenar los datos que despues seran enviados al objeto de estadisticas*/
var republicans = 0;
var democrats = 0;
var independents = 0;
var percentageRepublicans;
var percentageDemocrats;
var percentageIndependents;

/*Recorremos los senadores para ver cuantos corresponden a cual partido y los almacenamos en una variable, la cual, asignamos despues al equivalente en el objeto de estadisticas*/
senators.forEach(members =>{
  switch (members.party) {
    case "R" :
      republicans++;
      statistics.quantityRepublicans = republicans;
    break;
    case "D" :
      democrats++;
      statistics.quantityDemocrats = democrats;
    break;
    case "I" :
      independents++;
      statistics.quantityIndependents = independents;
    break;
  }
});
/*Sacamos el porcentaje de votos por partido*/
  percentageRepublicans = republicans*105/100;
  statistics.percentageOfVotesPerRepublicans = percentageRepublicans;
  percentageDemocrats = democrats*105/100;
  statistics.percentageOfVotesPerDemocrats = percentageDemocrats;
  percentageIndependents = independents*105/100;
  statistics.percentageOfVotesPerIndependents = percentageIndependents;
/*Diseñamos tabla dinamica de los partidos*/
  var partyTable = " ";
  partyTable += `<tr><td>${statistics.partyName1}</td>
  <td>${statistics.quantityRepublicans}</td>
  <td>${statistics.percentageOfVotesPerRepublicans}</td></tr>
  <tr><td>${statistics.partyName2}</td>
  <td>${statistics.quantityDemocrats}</td>
  <td>${statistics.percentageOfVotesPerDemocrats}</td></tr>
  <tr><td>${statistics.partyName3}</td>
  <td>${statistics.quantityIndependents}</td>
  <td>${statistics.percentageOfVotesPerIndependents}</td></tr>`
  document.querySelector('#partyTable').innerHTML=partyTable;
/*Recorremos los datos para obtener el top 10 de menor y mayor asistencia*/
    var members = [senators]
    var membersWithMostAttendance = [];
    members[0].sort((a,b)=>{
        return a.missed_votes_pct - b.missed_votes_pct
    })
    for (let i = 0; i < 10; i++) {
        membersWithMostAttendance.push(members[0][i]);
        statistics.tenWithMostAttendance = membersWithMostAttendance;
    }
    var membersWithLeastAttendance = [];
    members[0].sort((a,b)=>{
        return b.missed_votes_pct -a.missed_votes_pct
    })
    for (let i = 0; i < 10; i++) {
        membersWithLeastAttendance.push(members[0][i]);
        statistics.tenWithLeastAttendance = membersWithLeastAttendance; 
    }
    /*Recorremos los arrays de objetos y armamos las tablas dinamicas*/
    var tenWithMostAttendanceTable = " ";
    statistics.tenWithMostAttendance.forEach(members=>{
        tenWithMostAttendanceTable += `<tr><td><a href=${members.url}>${members.first_name} ${members.middle_name || ""} ${members.last_name}</a></td>
        <td>${members.missed_votes}</td>
        <td>${members.missed_votes_pct}</td></tr>`
    });
    document.querySelector('#mostEngagedTable').innerHTML = tenWithMostAttendanceTable;
    var tenWithLeastAttendanceTable = " ";
    statistics.tenWithLeastAttendance.forEach(members =>{
        tenWithLeastAttendanceTable += `<tr><td><a href=${members.url}>${members.first_name} ${members.middle_name || ""} ${members.last_name}</a></td>
        <td>${members.missed_votes}</td>
        <td>${members.missed_votes_pct}</td></tr>`
    })
    document.querySelector('#leastEngagedTable').innerHTML = tenWithLeastAttendanceTable;
    /*Recorremos los datos para obtener el top 10 de mas y menos leales*/
    var membersWithMostLoyalty = [];
    members[0].sort((a,b) => {
      return a.votes_against_party_pct - b.votes_against_party_pct
    })
    for (let  i = 0; i < 10; i++) {
      membersWithMostLoyalty.push(members[0][i]);
      statistics.tenWithMostLoyalty = membersWithMostLoyalty;
    }
    /*Recorremos los arrays de objetos (Loyalty) y armamos tablas dinamicas*/
    var tenWithMostLoyaltyTable = " ";
    statistics.tenWithMostLoyalty.forEach(members =>{
      tenWithMostLoyaltyTable += `<tr><td><a href=${members.url}>${members.first_name} ${members.middle_name || ""} ${members.last_name}</a></td>
      <td>${members.votes_against_party_pct}</td>
      <td>${members.total_votes}</td></tr>`
    });
    document.getElementById('tenWithMostLoyalty').innerHTML = tenWithMostLoyaltyTable;